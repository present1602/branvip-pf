...
class UserService {
  ...
  async getMyTrademarks(userId: string) {
    const applicants = await db.userApplicant.findMany({
      where: {
        userId,
        isVerification: true
      },
    ...
    if (applicants.length > 0) {
      let responseTrademarkList: any = []
      const promises = applicants.map(async (applicant: UserApplicant) => {
        if (!applicant.applicantNumber) return

        const kiprisTrademarkData = await kiprisService.getTrademarkesBySearch({
          keyword: applicant.applicantNumber,  // 샘플특허고객번호  keyword: "420070496201", "420220879554"
          offset: 0,
          limit: 50,
        });

        return kiprisTrademarkData.trademarks;
      })
      const results = await Promise.all(promises);
      responseTrademarkList = results.flat();
      return responseTrademarkList;
    } else {
      return []
    }
  }
